...
class LCACalculator:
    ...
    def calculate_impacts(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, row in data.iterrows():
            ...
            impacts = {
                ...
                'waste_generated (kg)': row['waste_generated_kg'],
                
                # Calculated impacts using impact factors, with units in column names.
                'carbon_impact (kg CO2e)': (
    ...
    def calculate_total_impacts(self, impacts: pd.DataFrame) -> pd.DataFrame:
        ...
        total_impacts = impacts.groupby(['product_id', 'product_name']).agg({
            'carbon_impact (kg CO2e)': 'sum',
            'energy_impact (MJ)': 'sum',
            'water_impact (L)': 'sum',
            'waste_generated (kg)': 'sum'
        }).reset_index()
    ...
    def normalize_impacts(self, impacts: pd.DataFrame) -> pd.DataFrame:
        ...
        impact_columns = ['carbon_impact (kg CO2e)', 'energy_impact (MJ)', 'water_impact (L)']
    ...
    def compare_alternatives(self, impacts: pd.DataFrame, product_ids: List[str]) -> pd.DataFrame:
        """
        Compare the environmental impacts between alternative products.
        ...
        Returns:
            DataFrame with relative percentage differences from the best performer
        """
        # Filter and aggregate the total impacts for the selected products
        product_summary = impacts[impacts['product_id'].isin(product_ids)].groupby(['product_id', 'product_name']).agg({
            'carbon_impact (kg CO2e)': 'sum',
            'energy_impact (MJ)': 'sum',
            'water_impact (L)': 'sum'
        }).reset_index()
        ...
        comparison = product_summary[['product_name']].copy()
        comparison.rename(columns={'product_name': 'Product'}, inplace=True)

        # Calculate relative differences for each impact type
        impact_columns = ['carbon_impact (kg CO2e)', 'energy_impact (MJ)', 'water_impact (L)']
        for impact_col in impact_columns:
            min_value = product_summary[impact_col].min()
            
            if min_value > 0:
                comparison[f'{impact_col}_relative_%'] = (
                    (product_summary[impact_col] - min_value) / min_value * 100
                ).round(1)
            else:
                comparison[f'{impact_col}_relative_%'] = 0.0
        
        return comparison
    
    def compare_materials(self, impacts: pd.DataFrame, material_1: str, material_2: str) -> pd.DataFrame:
        """
        Compare total carbon, energy, and water impacts between two materials.

        Args:
            impacts: DataFrame with calculated impacts
            material_1: Name of the first material (case-insensitive)
            material_2: Name of the second material (case-insensitive)

        Returns:
            DataFrame with total impacts for each material
        """
        # Normalize material names for comparison
        material_1 = material_1.lower()
        material_2 = material_2.lower()

        # Filter impacts for the two materials
        filtered = impacts[impacts['material_type'].str.lower().isin([material_1, material_2])]

        # Group by material and sum impacts
        summary = filtered.groupby('material_type').agg({
            'carbon_impact (kg CO2e)': 'sum',
            'energy_impact (MJ)': 'sum',
            'water_impact (L)': 'sum'
        }).reset_index()
        ...
        summary = summary.rename(columns={
            'material_type': 'Material',
            'carbon_impact (kg CO2e)': 'Total Carbon (kg CO2e)',
            'energy_impact (MJ)': 'Total Energy (MJ)',
            'water_impact (L)': 'Total Water (L)'
        })
