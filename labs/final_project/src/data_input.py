...
class DataInput:
    ...
    def validate_data(self, data: pd.DataFrame) -> bool:
        ...
        try:
            ...
            numeric_columns = ['quantity_kg', 'energy_consumption_kwh', 'transport_distance_km',
            ...
            for col in rate_columns:
                if not ((data[col] >= 0) & (data[col] <= 1)).all():
                    return False
            
            # Validate sum of rates equals either 0 (for non-disposal stages) or 1 (for disposal stages)
            rates_sum = data[rate_columns].sum(axis=1)
            valid_sums = (np.isclose(rates_sum, 0.0, rtol=1e-5) | np.isclose(rates_sum, 1.0, rtol=1e-5))
