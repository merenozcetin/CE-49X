...
def test_normalize_impacts(sample_data, impact_factors, tmp_path):
    ...
    assert all(normalized[col].max() <= 1 for col in impact_columns)
    assert all(normalized[col].min() >= 0 for col in impact_columns)
...
def test_compare_alternatives(sample_data, impact_factors, tmp_path):
    ...
    assert all(col in comparison.columns for col in [
        'Product', 'carbon_impact (kg CO2e)_relative_%', 
        'energy_impact (MJ)_relative_%', 'water_impact (L)_relative_%'
    ])

def test_compare_materials(sample_data, impact_factors, tmp_path):
    ...
    calculator = LCACalculator(impact_factors_path=impact_file)
    impacts = calculator.calculate_impacts(sample_data)
    comparison = calculator.compare_materials(impacts, 'steel', 'aluminum')
    
    assert len(comparison) == 2
    assert all(col in comparison.columns for col in [
        'Material', 'Total Carbon (kg CO2e)', 'Total Energy (MJ)', 'Total Water (L)'
    ]) 
